...
export interface WebcamDeviceConfig {
    /** 灯光名称 */
    name: string;
    /** 灯光位置 */
...
export class WebcamDeviceScript extends ScriptBase {
    private loadModel: GLBLoaderScript | null = null;
    private configs: WebcamDeviceConfig[] = [];
    ...
    constructor(loadModel: GLBLoaderScript, configs?: WebcamDeviceConfig | WebcamDeviceConfig[]) {
        super();
        this.name = "WebcamDeviceScript";
     ...
     * @param config 天花板灯配置
     */
    private async loadModelByConfig(config: WebcamDeviceConfig): Promise<void> {
        if (!this.loadModel) {
