...
export interface RobotVacuumCleanerScriptConfig {
    name: string;
...
export class RobotVacuumCleanerScript extends ScriptBase {
    private loadModel: GLBLoaderScript | null = null;
    private configs: RobotVacuumCleanerScriptConfig[] = [];
    ...
    constructor(loadModel: GLBLoaderScript ,configs?: RobotVacuumCleanerScriptConfig | RobotVacuumCleanerScriptConfig[]) {
        super();
    ...
    public start(): void {
        ...
        this.engine()?.on('mouse:objectSelected', (data) => {
            ...
            if (object && object.name === 'Obj3d66-14820007-58-339') {
     ...
     * @param config 天花板灯配置
     */
    private async loadModelByConfig(config: RobotVacuumCleanerScriptConfig): Promise<void> {
        if (!this.loadModel) {
        ...
        this.scene.traverse((object: THREE.Object3D) => {
            if (!object) return;
            if (object.name === 'Obj3d66-14820007-56-509' && object instanceof THREE.Mesh) {
            ...
            if (object.name === 'Obj3d66-14820007-58-339' && object instanceof THREE.Mesh) {
                const geometry = object.geometry;
                if (!geometry.attributes && !geometry.attributes.uv1) return;
