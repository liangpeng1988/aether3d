...
// @ts-ignore
import { Aether3d, THREE, OrbitControlsScript, ScriptBase } from '../../dist/engine/aether3d-engine.es.js';

// 自定义设备类
class CustomDeviceScript extends ScriptBase {
  name: string;
  ...
  windColor: string = '#00ff00';
  
  // 设备对象
  deviceObject: THREE.Group | null = null;
  blade1: THREE.Mesh | null = null;
  blade2: THREE.Mesh | null = null;

  constructor(name: string, position: THREE.Vector3, rotation: THREE.Euler, scale: THREE.Vector3) {
    super();
  ...
  toggleDevice() {
    ...
    if (this.deviceObject) {
      ...
      (top as THREE.Mesh).material = new THREE.MeshPhongMaterial({ 
  ...
  changeWindColor() {
    ...
    if (this.blade1 && this.blade2) {
      (this.blade1.material as THREE.MeshPhongMaterial).color.set(randomColor);
      (this.blade2.material as THREE.MeshPhongMaterial).color.set(randomColor);
...
export default CustomDeviceScript;